...
def predict(command: str):
    try:
        ...
        data = MedLMOutput(**result.json())
    except Exception as e:
        logger.error(f"Error predicting with MedLM: {e}")
        raise e
...
def verify_results_using_medlm(
    ...
    for result in gemini_result:
        ...
        if i == o:
            ...
            clean_mapping.append({"input": i, "output": o, "output_medlm": response})
    clean_mapping_df = pd.json_normalize(clean_mapping, sep="_")
    clean_mapping_df.loc[clean_mapping_df["output_medlm_flag"] == 0, "output"] = (
        clean_mapping_df.loc[clean_mapping_df["output_medlm_flag"] == 0, "input"]
    )
